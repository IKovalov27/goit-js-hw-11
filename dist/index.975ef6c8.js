...
let pageToFetch;
const pageLimit = 40;
const fetchImages = async (queryToFetch1, pageToFetch1)=>{
...
const getImages = async (query, pageToFetch2)=>{
    try {
        const data = await fetchImages(query, pageToFetch2);
        if (!data.hits.length) {
            buttonLoadMore.classList.replace("load-more", "is-hidden");
        ...
        renderImages(data);
        lightbox.refresh();
        if (pageToFetch2 === 1) (0, _notiflixDefault.default).Notify.success(`Horay! We found ${data.totalHits} images.`);
        if (data.totalHits >= pageToFetch2 * pageLimit) buttonLoadMore.classList.replace("is-hidden", "load-more");
        if (data.totalHits <= pageToFetch2 * pageLimit) {
            buttonLoadMore.classList.replace("load-more", "is-hidden");
            (0, _notiflixDefault.default).Notify.info("We're sorry, but you've reached the end of search results");
        }
    } catch (error) {
        console.log(error);
        (0, _notiflixDefault.default).Notify.failure("Ooops! Something went wrong!");
    }
};
function onSubmitForm(event) {
    event.preventDefault();
    const query = event.currentTarget.elements.searchQuery.value;
    if (!query.trim() || query === queryToFetch) return;
    queryToFetch = query;
    galleryEl.innerHTML = "";
    pageToFetch = 1;
...
},{"notiflix":"5z0Oc","simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","axios":"jo6P5","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5z0Oc":[function(require,module,exports) {
var global = arguments[3];
